...
class UniElement {
  ...
  _getBoundingClientRectAsync(callback) {
    const query = index$1.createSelectorQuery().in(this.$vm);
...
function initMiniProgramNode(uniElement, ins) {
  if (uniElement.tagName === "SCROLL-VIEW") {
    uniElement.$node = new Promise((resolve2) => {
      setTimeout(() => {
        index$1.createSelectorQuery().in(ins.proxy).select("#" + uniElement.id).fields({ node: true }, (res) => {
          const node = res.node;
          resolve2(node);
          uniElement.$node = {
            then(fn) {
...
const getElementById = defineSyncApi(API_GET_ELEMENT_BY_ID, (id) => {
  ...
  return findUniElement(id, page2.$vm.$);
});
...
function createUTSJSONObjectIfNeed(obj) {
  if (!isPlainObject$2(obj) && !Array.isArray(obj)) {
    return obj;
  }
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__2049EF1",
    appName: "mvr-rental-platform",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function initRuntimeSocketService() {
  const hosts = "127.0.0.1,192.168.110.47,26.26.26.1";
  const port = "8090";
  const id = "mp-weixin_ogbxw8";
...
function arrayPop(array2) {
  if (array2.length === 0) {
    return null;
  }
  return array2.pop();
}
function arrayShift(array2) {
  if (array2.length === 0) {
    return null;
  }
  return array2.shift();
}
function arrayFind(array2, predicate) {
  const index2 = array2.findIndex(predicate);
  if (index2 < 0) {
    return null;
  }
  return array2[index2];
}
function arrayFindLast(array2, predicate) {
  const index2 = array2.findLastIndex(predicate);
  if (index2 < 0) {
    return null;
  }
  return array2[index2];
}
function arrayAt(array2, index2) {
  if (index2 < -array2.length || index2 >= array2.length) {
    return null;
  }
  return array2.at(index2);
}
var IDENTIFIER;
...
function parseObjectOrArray(object2, utsType) {
  const objectType = getType(object2);
  if (object2 === null || objectType !== "object" && objectType !== "array") {
    return object2;
  }
  if (utsType && utsType !== UTSJSONObject) {
    try {
      return new utsType(object2, void 0, true);
    } catch (error2) {
      console.error(error2);
      return null;
    }
  ...
  if (objectType === "array") {
    return object2.map((value) => {
      return parseObjectOrArray(value);
    });
  } else if (objectType === "object") {
    return createUTSJSONObject(object2);
  }
  return object2;
}
const UTSJSON = {
